...
import { Mollie } from "../../lib/api";
import { MolliePayment } from "../../lib/collections/schemas";
...
function handleMollieSubmitError(error) {
...
AutoForm.addHooks("mollie-payment-form", {
  onSubmit(doc) {
    submitting = true;
    const { template } = this;
    hidePaymentAlert();
    ...
    const packageData = Packages.findOne({
      name: "mollie-paymentmethod",
      shopId: Reaction.getShopId()
    });
    Mollie.authorize(form, {
      ...
      if (error) {
        ...
        handleMollieSubmitError(transaction.error);
        uiEnd(template, "Resubmit payment");
      }
    });
    return false;
